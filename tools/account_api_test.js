...
(async () => {
  ...
  r = await api('POST', '/api/password', { pass: 'nope', newPass: 'hunter33' }, tok2);
  check('password change wrong old', r.status === 401);
  r = await api('POST', '/api/password', { pass: 'hunter22', newPass: 'hunter33' }, tok2);
  check('password change ok', r.status === 200 && r.j.token);
  const tok3 = user + ':' + r.j.token;
  r = await api('GET', '/api/profile', null, tok2);
  check('old token dead after password change', r.status === 401);
  r = await api('POST', '/api/login', { user, pass: 'hunter33' });
  check('login with new password', r.status === 200 && r.j.admin === false);
  const tok4 = user + ':' + r.j.token;
  r = await api('POST', '/api/password', { pass: 'hunter33', newPass: 'hunter22' }, tok4);
  check('password back', r.status === 200);
  r = await api('POST', '/api/login', { user, pass: 'hunter22' });
  const tok5 = user + ':' + r.j.token;
  r = await api('POST', '/api/logout', null, tok5);
  check('logout', r.status === 200 && r.j.ok);
  r = await api('GET', '/api/profile', null, tok5);
