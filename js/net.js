...
(function (global) {
  ...
  Net.prototype._setupHost = function (profile, code) {
    ...
    this.players = { 0: { id: 0, name: profile.name, char: profile.char, pet: profile.pet, kart: profile.kart || 'nitro', skin: profile.skin || 'classic' } };
  };
  ...
  Net.prototype._onHostData = function (conn, msg) {
    ...
    if (msg.t === 'hello') {
      ...
      this.players[id] = { id: id, name: String(msg.name || 'PLAYER').slice(0, 12), char: msg.char || 'volt', pet: msg.pet || null, kart: msg.kart || 'nitro', skin: typeof msg.skin === 'string' ? msg.skin.slice(0, 32) : 'classic' };
  ...
  Net.prototype._join = function (code, profile, cb) {
    ...
    peer.on('open', function () {
      ...
      conn.on('open', function () { self._emit('stage', 'open'); conn.send({ t: 'hello', name: profile.name, char: profile.char, pet: profile.pet, kart: profile.kart, skin: profile.skin }); });
    ...
    ws.onmessage = function (ev) {
      ...
      if (c0 === 33) {                                         // !ok|CODE — room exists, host reachable; !bye|why — closed by server
        if (s.slice(1, 3) === 'ok') {
          ...
          conn.send({ t: 'hello', name: profile.name, char: profile.char, pet: profile.pet, kart: profile.kart, skin: profile.skin });
