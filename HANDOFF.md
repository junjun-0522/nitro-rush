# NITRO RUSH — 작업 인수인계 (2026-09-05, 친구·팀전·타임어택 반영)
...
## 현재 상태 (전부 동작·배포됨)
- 트랙 11개(모드: ITEM+SPEED·SPEED·ITEM·TIME ATTACK, 팀전 토글): AZURE COAST(입문), NEON METROPOLIS(고수), SUNSET CANYON(중급), FROST PEAK(고수·빙판 그립 0.86), **SEOUL BLOSSOM(중급, 한국 테마: 광화문·경복궁·한강 대교/잠수교·롯데타워·63빌딩·DDP·남산 N타워·북촌 한옥·벚꽃길·태극기)**
...
- 모드 4개: ITEM + SPEED(기본) / SPEED RACE(아이템 없음, 부스터 2개 저장, 게이지 1.5배) / ITEM RACE(게이지·드리프트 부스터 없음) / TIME ATTACK(혼자, 고스트) + 팀전 토글(모든 레이스 모드와 조합)
- 카트 16종(`js/karts.js`: nitro/bullet/bigfoot/formula/hover/geobukseon + 2026-09-08 신형 10종 — 차체 빌드 함수 + 능력치 배수 + 질량 + 바퀴 배치) × 캐릭터 8 + 펫 6 (`js/chars.js`). GARAGE 화면에서 선택, 온라인 프로필(hello/players)에 kart 포함, AI는 공유 RNG로 랜덤(aiLook). 능력치 = 카트 × 캐릭터 × 펫.
...
- (2026-09-07) 최대 레벨 100(`MAX_LEVEL`, 커브 동일 → Lv100 = 186,642 XP), 칭호 50 MASTER/60 ELITE/70 TITAN/80 MYTHIC/90 IMMORTAL/100 INFINITY, 스킨 32종(새 패턴 lava/bolt/checker/holo/circuit/scales; 트랙 우승 스킨 lab/orbit/sky/volcano, 레벨 25·40·50·60·70·80·90·100, 50승 DRAGON, 200완주 PHANTOM, 온라인 30회 SQUAD, MAX 드리프트 300회 THUNDER).
- **스킨 32종** `js/skins.js` (classic 기본 + 레벨/우승/완주/온라인/MAX드리프트/서울 우승/전 트랙 포디움 해금). 스킨 = paint/accent 색 + 캔버스 패턴(stripes/flames/camo/carbon/galaxy/petals/neon/pixel/taeguk/hex) + metal/rough/emissive/glow. `buildKartMesh(..., skinId)` 가 적용, `Kart.skinId`, 온라인 hello/players 에 `skin` 포함, AI 는 aiLook 에서 30% 확률로 랜덤 스킨(공유 RNG 소비 순서 유지!). GARAGE 맨 위 SKIN · INVENTORY 카드(잠김 카드는 해금 조건+진행 바, 클릭 시 흔들림).
...
## 2026-09-10 추가: GRAND MANSION (11번째, 순환 3랩 2.3km, INTERMEDIATE)
- 사용자 요청: 카트라이더 대저택 테마, 모기만 한 크기로 집 안 투어. 레이아웃 `tools/layout.js mansion`(순환, 클로저 614m 가 갤러리 구간). 홀 로툰다(난간 링·우물·샹들리에·아치 창 26개·돔) → 카펫 복도 → 거대 책장 도서관(시계·지구본) → 책 계단(profile 0→22) → 독서 테이블 위(촛대 헤어핀·찻잔·펼친 책·잉크) → 테이블 끝 램프(ramps h3) 점프 → 소파(y8) → 러그 경사 → 식탁 밑(다리·의자·촛대) → 벽난로(불꽃 스프라이트·포인트라이트) → 초상화 갤러리. 텍스처 parquet/carpet/books/wallpaper(util.js). 카펫은 `track.strip` 데칼, 벽은 `wallRun`, 외벽 4면.
- 함정: `g.add(mesh(...)).rotation.x = 0.4` 처럼 `add()` 반환값(그룹)에 회전을 걸면 트랙 그룹 전체가 기울어짐(물리는 path 기준이라 화면만 어긋남). 항상 변수에 받아서 회전할 것. 거대 가구/벽/천장은 castShadow 끔(방 전체가 그림자로 검게 됨).

## 2026-09-08 추가: 세련된 카트 16종 · 모바일 터치 조작 · 화질
- 사용자가 카트라이더 러쉬플러스 사진을 보내며 "카트 세련되게 + 더 많이, 모바일 클릭 조작, 화질 향상"을 요청.
...
- 2026-09-09 바이크 2종(WIND RIDER 영웅 Lv30, THUNDER HOG 희귀 Lv18): `def.bike`(핸들 링 대신 핸들바), `def.lean`(코너 안쪽으로 기울기, 카트는 바깥쪽 롤), 바퀴 pos 항목의 3번째 `true` = 메시 없는 피벗(스키드/물리용 4피벗 유지, 바퀴 메시 2개). 카트 총 19종.
...
- 사용자 영어 스펙: 비순환·랩 없음·체크포인트 진행(cpCount 8), 황무지 → 협곡(갈림길 2개) → 용암 폭포 → 낙석 붕괴 → 화산 등반 → 대분화(도로 붕괴 + 새 길) → 용암 다리(틈 점프 3개) → 최종 용암 대점프 → 안전 고원 FINISH. 레이아웃 `tools/layout.js volcano`(profile 로 고도: 등반 0→72, 하강 44, 점프 후 24/20).
- **엔진 추가**: `def.gaps=[{a,b,h,retry,big}]` → `gapMask`(도로/레일/포스트/교각 없음, `groundY` 는 노면 -40 = 용암), 램프는 자동 생성(hExtra), 카트가 틈 안에서 노면 -2.5m 아래로 떨어지면 `Kart.lavaDeath` → `gap.retryS`(부스트 패드 앞) 에서 복귀 + 'lava' 이벤트("FELL INTO LAVA!"). `Track.setLimits(a,b,min,max,ramp)` 동적 주행 한계(깔때기), `track.obstacles`(단단한 장애물 {x,y,z,r,s,lat,off}: hazards.js 가 카트 충돌, ai.js 가 회피), `Track.strip()`, `ribbonGeom()`(용암 강), `track.focus`(플레이어 위치, 재/불티 파티클이 따라다님), `track.skyDome` 참조(분화 시 하늘 색 변경).
- **점프 물리 버그 수정**: 이전엔 `climb` 이 항상 0 이라 램프에서 공중에 뜬 적이 없었음(노면 하강 시 그냥 스냅). 이제 `this.prevY`(전 프레임 정착 높이) 로 상승 속도를 계산해 램프 끝에서 발사되고, 절벽에서 떨어지면 낙하함. 모든 트랙의 램프에 영향(회귀 테스트 통과).
- **hazards.js**(`def.hazards`): 낙석(경고: 먼지·자갈·붉은 링·진동 → 낙하 → 착지 후 장애물, `behind` 는 플레이어 뒤에 연출용), 자갈 굴러감(시각), 도로 균열 데칼, 절벽 붕괴 덩어리, 용암 분출(거품 경고 → 분출 시 스핀, 분출 중엔 AI 장애물), 진동+원거리 분출, **분화 이벤트**(어느 카트든 trigger 통과 시: 진동·분수·연기 구름·조명 붉게(`hazards.light`→game.js `applyLighting`)·카메라 이벤트·타깃 용암탄 → 1.4s 부터 옛 산길(왼쪽 zone 'oldroad') 슬래브가 용암으로 가라앉고 용암 흐름이 덮임 → 2.0s `setLimits` 로 새 길(본선)만 주행 가능 → 2.4s 바위 벽 폭파 + "NEW ROUTE →"), 다리 진동, 폭포 불티, 분화 후 유성. 분화 전에는 `setLimits` 로 옛 길만 열려 있음(본선은 바위 벽).
- 카메라: 분화 시 3.6s 동안 뒤로 빠지며 분화구 쪽으로 시선(`camEvent`), 최종 점프 공중에서 뒤/위로 빠짐(`camJump`), 조작엔 영향 없음.
- 튜닝 근거: 최종 점프 램프 h9/40m, 틈 52m, 착지 18m 아래 → 36m/s 이상이면 성공(부스트 패드 3개). 다리 틈 7m(램프 h1.2) → 23m/s 이상.
- 테스트: `?autotest&norender&steps=4&track=9&mode=speed` 로 8대 중 5~8대 완주(2:03~2:30), 이벤트 로그 `[AUTOTEST] hazard ...`. `holdAt=0.69` 파라미터로 그 지점에서 루프 정지(스크린샷용). 정적 뷰: `tools/track_view.html?track=9&s=0.78`. 하네스 `EVAL_OUT=파일` 로 eval 결과를 파일에 저장(캔버스 toDataURL 덤프 등).
- 미확인: 실제 플레이 난이도(낙석 타이밍, 분출 빈도, 갈림길 폭), 온라인에서 분화 타이밍(각 클라이언트 로컬 시뮬).

## 2026-09-05 추가: 타임어택 · 팀전 · 친구
- **타임어택** (`MODES.time`, `solo: true`): 메인 메뉴 ⏱ Time Attack 버튼(모드를 `time` 으로 바꾸고 트랙 선택으로) 또는 Mode 버튼 순환. 플레이어 혼자(n=1), 아이템 없음, 스피드전 규칙(부스터 2개·게이지 1.5배). 기록은 `records[track].time` 에 따로 저장. **고스트** `js/ghost.js`: 15Hz 로 x/y/z/yaw/진행도/플래그(드리프트·부스트·에어·비행) 를 Float32 → base64 로 localStorage `nitroRush.ghost.v1.<trackId>` 에 저장(트랙당 1개, 이 기기 전용, 계정 동기화 안 됨). 신기록일 때만 저장(`onPlayerFinish`). 다음 주행에서 반투명 카트(`makeGhostKart`, 재질 clone opacity 0.42, `karts` 배열에 안 넣어 충돌/순위 무관) 가 재생되고 HUD 우측 `#hudTA` 에 RECORD·고스트 대비 ±초(진행도 기준 `timeAtProgress`)·랩 스플릿, 미니맵에 흰 점. 결과 화면은 랩 테이블(고스트 랩 대비), `Progress.award` 는 `ctx.solo` 면 순위/우승 보상·wins/podiums 카운트 없음(타임어택 완주 90 + 고스트 격파 40 + 신기록 100). 온라인 로비는 `onlineMode()` 로 solo 모드 제외.
- **팀전** (`settings.team`, 로비 `lobby.team`, cfg `team`): RED(0)/BLUE(1). 솔로는 플레이어 RED + AI 번갈아, 온라인은 플레이어 `p.team`(기본 id 홀짝, 로비 `팀 바꾸기` 버튼 → 클라이언트는 `{t:'team'}` 을 호스트에 보내고 호스트가 `players[id].team` 토글 후 `setLobby` 로 방송) + AI 채움은 인원 적은 팀부터. `Kart.team`, 팀 카트는 `accent`(바닥 글로우) 팀색, AI/상대 머리 위 팀색 삼각형 스프라이트(`makeTeamMarker`), 미니맵·HUD 보드·결과 팀색. 점수 `TEAM_PTS=[10,8,7,6,5,4,3,2]`(9위부터 1) 순위 합산, HUD `#hudTeam` 바(0.3s 갱신), 결과 `TEAM VICTORY!/DEFEAT/DRAW!` + RED n : n BLUE + 포인트 열, XP `teamWin` +60. **팀원끼리 로켓 타깃/피격·트랩·라이트닝 면제**(`items.js sameTeam`). 자동테스트 URL `&team`.
- **친구 + 온라인 표시** 서버 `relay/src/social.js`: 전역 Durable Object `Social`(이름 'global', 마이그레이션 v3, 바인딩 SOCIAL). 라우팅은 `account.js handleApi` 가 `/api/friends*`, `/api/presence` 를 Bearer 토큰 → 해당 유저 Account DO `verify` op 로 확인 후 `handleSocial` 로 넘김(요청 대상 존재 확인은 Account DO `exists` op). 저장 키 `f:/in:/out:/name:/pres:/rl:`. 친구 최대 50, 대기 30, 요청 10회/10분, 온라인 판정 = 마지막 하트비트 100초 이내. 역방향 요청은 자동 수락, decline 은 보낸 요청 취소도 겸함. 클라이언트 `js/social.js`: 로그인 상태에서 40초마다 + 상태 변경 시 `POST /api/presence {state, room, host}` → 응답이 친구 목록(온라인·상태·방 코드) 이라 별도 폴링 없음. 상태는 `game.js updatePresence(screen)`: menu/lobby(방 코드)/racing/timeattack/garage. 탭이 숨겨지면 하트비트 생략. UI: 메인 메뉴 프로필 바 `👥 친구 n · 온라인 n`(받은 요청 수 배지) → Friends 화면(아이디로 요청, 받은 요청 수락/거절, 친구별 초록/노랑 점 + 상태 문구, 친구가 방 대기 중이면 **참가 CODE** 버튼 → `joinFriendRoom` 이 닉네임 채우고 online 화면으로 가서 `joinRoom`). 테스트 `node tools/social_api_test.js <url>` 29개, 브라우저 자동테스트 파라미터 `friends`(화면 열기) · `login=아이디:비번`(zsh 에선 `${H}:pw` 로 써야 함 — `$H:h` 는 zsh 수정자). 2인 친구 참가 플로우 검증 스크립트는 세션 스크래치에 있었음(호스트 `online=host&login=` + 게스트 `friends&login=` 후 `[data-act=join]` 클릭).
- 자동테스트에서는 `visibilitychange` 일시정지를 건너뜀(CDP 키 입력 시 headless 가 hidden 이 되어 게임이 멈추던 문제).
...
- 배포: `git add -A && git commit && git push origin main` (커밋 서명은 Co-Authored-By: Claude 형식 유지). 릴레이/계정 서버는 `cd relay && npx wrangler@4 deploy` 별도 배포 (DO 마이그레이션 v1 Room, v2 Account, v3 Social).
- 친구 API 테스트: `node tools/social_api_test.js http://127.0.0.1:8787` (29개) — 로컬은 `cd relay && npx wrangler@4 dev --port 8787` 띄운 뒤.

## 코드 구조 (`js/`)
config(relayUrl/accountUrl/ICE/TURN) · karts(카트 차체) · skins(스킨·패턴) · progress(XP·기록·해금) · account(클라우드 세이브 클라이언트) · social(친구·온라인 표시) · ghost(타임어택 고스트) · util(텍스처·RNG) · audio(Web Audio SFX/BGM) · path(스플라인, 뱅크) · tracks(트랙 정의·도로·경관) · chars(캐릭터·펫) · kart(메시·물리·원격 보간) · fx(파티클·스키드) · items · ai · net(릴레이 WS 또는 PeerJS) · game(상태·UI·HUD·온라인 통합)
